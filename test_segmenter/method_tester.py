...
def test_segments_by_combining_sheet_paragraphs():
    ...
    for sheet in excel_data.sheet_names:
        ...
        file_name = 'segmenter/parsed_sentences/'+ sheet +'sheet_parsed.txt'
        file2  = open(file_name, "w",  encoding='utf-8')
        #works for both sentences and paragraphs
        file2.write(final_sentences)
        file2.close()

        md_file_path = root_dir_path+ "/test_segmenter/files_for_ground_truth/" + sheet
        print(f"Segmenting {md_file_path}")
        predicted_segmentation, segments, segmented_file_path = segment(sentence_feature_extractor, md_file_path, out_filename='new_contributing_kubernetes', save_to_file=True, segmentation_method='unsupervised_window_based', sentence_method= 'stanza')
